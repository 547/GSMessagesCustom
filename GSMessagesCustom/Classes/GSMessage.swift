...
///相对于text的位置
public enum GSMessageImagePosition {
    case leftTop
    case leftCenter
    case leftBottom
    case rightTop
...
public enum GSMessageOption {
    ...
    case imagePosition(GSMessageImagePosition)
    case customBackgroundColor(UIColor)
}

extension UIViewController {
    
    open func showMessage(_ text: String, image:UIImage? = nil,
                          type: GSMessageType,
                          options: [GSMessageOption]? = nil) {
        
        GSMessage.showMessageAddedTo(text: text, image: image, type: type, options: options, inView: view, inViewController: self)
    }

    open func showMessage(_ attributedText: NSAttributedString, image:UIImage? = nil,
                          type: GSMessageType,
                          options: [GSMessageOption]? = nil) {
        
        GSMessage.showMessageAddedTo(attributedText: attributedText, image: image, type: type, options: options, inView: view, inViewController: self)
    }
...
extension UIView {
    ...
    open func showMessage(_ text: String,
        ...
        GSMessage.showMessageAddedTo(text: text, image: image, type: type, options: options, inView: self, inViewController: nil)
    }
    ...
    open func showMessage(_ attributedText: NSAttributedString,
        ...
        GSMessage.showMessageAddedTo(attributedText: attributedText, image: image, type: type, options: options, inView: self, inViewController: nil)
    }
...
public class GSMessage: NSObject {
    ...
    public class func showMessageAddedTo(text: String,
        ...
        showMessageAddedTo(attributedText: attributedText, image:image, type: type, options: options, inView: inView, inViewController: inViewController)
    }
    ...
    public private(set) var messageImageView = UIImageView()
    ...
    public private(set) var imagePosition: GSMessageImagePosition = .leftTop
    public private(set) var imageViewSize = CGSize(width: 24, height: 24)
    public private(set) var imageAndTextSpace = CGFloat(10)
    public private(set) var customBackgroundColor: UIColor? = nil
    ...
    fileprivate var textWidth: CGFloat {
        var result = messageWidth - padding.horizontal
        if let _ = messageImageView.image {
            result -= imageViewSize.width + imageAndTextSpace
        }
        return result
    }
    ...
    public init(attributedText: NSAttributedString,
        ...
        for option in options ?? [] {
            ...
            case let .imagePosition(value): imagePosition = value
            case let .customBackgroundColor(value): customBackgroundColor = value
            }
        }
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(updateFrames), name: Notification.Name.UIDeviceOrientationDidChange, object: nil)
    ...
    @objc fileprivate func updateFrames() {
        ...
        if let attrText = messageText.attributedText {
            var size = CGSize(width: textWidth, height: 999)
            if let _ = messageImageView.image {
                size = CGSize(width: textWidth - imageViewSize.width - imageAndTextSpace, height: 999)
            }
        ...
        updateMessageFrame()
        updateCornerRadius()
        updateMessageContentFrameWithTextSize(textSize: textSize)
    }
    ...
    func updateMessageContentFrameWithTextSize(textSize: CGSize) {
        var imageViewX:CGFloat = 0
        var imageViewY:CGFloat = 0
        var imageViewWidth:CGFloat = 0
        var imageViewHeight:CGFloat = 0
        let imageViewMaxHeight:CGFloat = height - padding.vertical
        
        if let _ = messageImageView.image {
            imageViewWidth = imageViewSize.width
            imageViewHeight = imageViewSize.height
        }
        ...
        case .topLeft, .left, .bottomLeft:
            ...
            if let _ = messageImageView.image {
                ...
                case .leftTop, .leftCenter, .leftBottom:
                    textX = padding.left + imageViewWidth + imageAndTextSpace
                case .rightTop, .rightCenter, .rightBottom:
                    break
                }
            }
        case .topCenter, .center, .bottomCenter:
            textX = (messageView.frame.width - textW) * 0.5
        case .right, .topRight, .bottomRight:
            textX = messageView.frame.width - padding.left - textW
            if let _ = messageImageView.image {
                switch imagePosition {
                case .leftTop, .leftCenter, .leftBottom:
                    break
                case .rightTop, .rightCenter, .rightBottom:
                    textX = messageView.frame.width - padding.left - textW - imageViewWidth - imageAndTextSpace
                }
            }
        }
        if let _ = messageImageView.image {
            switch imagePosition {
            case .leftTop, .leftCenter, .leftBottom:
                imageViewX = textX - imageAndTextSpace - imageViewWidth
            case .rightTop, .rightCenter, .rightBottom:
                imageViewX = textX + textW + imageAndTextSpace
            }
            if imageViewX <= 0 || imageViewHeight > imageViewMaxHeight {
                imageViewWidth = 0
                imageViewHeight = 0
            }
        }
        ...
        case .leftCenter, .rightCenter:
            imageViewY = textY + textHeight * 0.5 - imageViewWidth * 0.5
        case .leftBottom, .rightBottom:
            imageViewY = textY + textHeight  - imageViewWidth
        }
        
        messageImageView.frame = CGRect(x: imageViewX, y: imageViewY, width: imageViewWidth, height: imageViewHeight)
...
@objc fileprivate extension GSMessage {
    ...
    func keyboardWillShow(notification: NSNotification) {
        ...
        if let keyboardSize = (notification.userInfo?[Notification.Name.UIKeyboardWillChangeFrame] as? NSValue)?.cgRectValue {
